...
class TFDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            # print('before', type(example), example.dtype, label.dtype, example.size, example.shape, example[0][0][0][0])
            times, channels, height, width = example.shape
            if times < 16:
                example = np.concatenate([example, example], axis=0)
            gap = int(times // 8)
            offset = random.randint(0, gap-1)
            index = [gap*i+offset for i in range(8)]
            # print('gap %d, offset %d, index%s' %(gap, offset, index))
            # index[0] = max(1, index[0])
            # example = np.concatenate([[example[i] - example[i-1]] for i in index], axis=0)
            index = [0] + index
            example = np.concatenate([[example[index[i+1]] - example[index[i]]] for i in range(len(index)-1)], axis=0)
            # example = np.concatenate([[example[i]] for i in index], axis=0)
           
            # print('after', type(example), example.dtype, label.dtype, example.size, example.shape, example[0][0][0][0])   
    ...
    def __getitem_scan__(self, index):
        session = self.session if self.session is not None else tf.Session()
        try:
            example, label = session.run(self.next_element)
            # print(type(example), example.dtype, label.dtype, example.size, example.shape, example[0][0][0][0])
        except tf.errors.OutOfRangeError:
            self.reset()
            raise StopIteration
        return example, label
    
    def scan_test(self, samples=1000000, device = None):
        tensors = []
        labels = []
    ...
    def scan(self, samples=10000000, with_tensors=False, is_batch=False, device=None, half=False, is_test = False, batch_size=32):
        ...
        for i in range(min(self.num_samples, samples)):
            try:
                if is_test:
                    ...
                    print(batch_label)
                    example = np.concatenate(batch_example)
                    label = np.concatenate(batch_label)
                    # example = torch.Tensor(batch_example)
                    # label = torch.Tensor(batch_label)

                else:
                    example, label = self.__getitem_scan__(i)
            except tf.errors.OutOfRangeError:
                break
            except StopIteration:
                break
        ...
        info = {
            ...
            'example': {
                ...
                'shape_min': [int(v) for v in np.min(shapes, axis=0)], 
                'shape_max': [int(v) for v in np.max(shapes, axis=0)], 
...
class TransformDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print(1, type(tensors[0]), tensors[0].shape,   tensors[0].dtype, tensors[0][0][0][0])
        ...
        # print(2, type(tensors[0]), tensors[0].device, tensors[0].shape,  tensors[0][0][0][0])
