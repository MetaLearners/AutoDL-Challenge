...
from speech_util.CONSTANT import MAX_FRAME_NUM, IS_CUT_AUDIO, MAX_AUDIO_DURATION, AUDIO_SAMPLE_RATE,VERBOSE
from speech_util.data_process import ohe2cat, get_max_length, pad_seq, extract_mfcc_parallel
from speech_util.models.my_classifier import Classifier
    ...
    def init_model(self,
        ...
        for layer in range(1, layers):
            ...
            x = Convolution2D(
                ...
                kernel_size=kernel_size,
                padding='same',
                name='conv'+str(layer + 1)+'_1')(x)
            x = ELU()(x)
            x = BatchNormalization(
                axis=channel_axis,
                name='bn'+str(layer + 1)+'_1')(x)

            x = Convolution2D(filters=filters_size[layer], kernel_size=kernel_size, padding='same',
                              name='conv'+str(layer + 1)+'_2')(x)
            x = ELU()(x)
            x = BatchNormalization(
                axis=channel_axis,
                name='bn'+str(layer + 1)+'_2')(x)

            if filters_size[layer] != 128:
                x = Convolution2D(filters=filters_size[layer], kernel_size=kernel_size, padding='same',
                                  name='conv'+str(layer + 1)+'_3')(x)
                x = ELU()(x)
                x = BatchNormalization(
                    axis=channel_axis, name='bn'+str(layer + 1)+'_3')(x)
