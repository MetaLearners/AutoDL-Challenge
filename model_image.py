...
class CVModel(LogicModel):
    ...
    def init_opt(self):
        ...
        conv_keys = [n for n, p in self.model.named_parameters() if p.requires_grad and (n[:5] =='layer' or n[:5]=='conv1' or n[:2]=='bn') ]
        ...
        self.lr_table = {
            'small_image':{
                'lr_simple':0.001, 'lr_fc_only':0.001, 'lr_fc':0.01, 'lr_conv':0.001
                },
            'large_image':{
                'lr_simple':0.05, 'lr_fc_only':0.025, 'lr_fc':0.03, 'lr_conv':0.01
                },
            'very_large_image':{
                'lr_simple':0.2, 'lr_fc_only':0.01, 'lr_fc':0.05, 'lr_conv':0.008
            }
        }

        if not self.is_multiclass():
            self.lr_table['very_large_image'] = {
                'lr_simple':0.01, 'lr_fc_only':0.01, 'lr_fc':0.01, 'lr_conv':0.0005
            }
    ...
    def build_optimizer(self):
        ...
        self.optimizer_simple = torch.optim.SGD(
            self.params_simple, lr=self.lr_table[self.image_mode]['lr_simple'], momentum=0.9, nesterov=False
            )
        ...
        self.optimizer_fc = torch.optim.SGD(
            self.params_fc, lr=self.lr_table[self.image_mode]['lr_fc_only'], momentum=0.9, nesterov=False
        )
        ...
        self.lr_schedular_all = torch.optim.lr_scheduler.ReduceLROnPlateau(
            self.optimizer_all, mode='max', factor=0.5, patience=5, verbose=True, min_lr=1e-8, cooldown=0, threshold=0.02
        )
