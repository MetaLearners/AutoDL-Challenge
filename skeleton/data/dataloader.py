...
class FixedSizeDataLoader:
    ...
    def change_steps(self, step_num=5):
        if self.steps + step_num < self.dataloader_len:
            self.steps  = self.steps + step_num

    def set_steps(self, step_num = 30):
        if step_num < self.dataloader_len:
            self.steps = step_num
