...
#             CRNN2D_MODEL
    ...
    def _get_or_create_model(self):
        ...
        log('This train loop use ' + str(self._model_name)+', last train loop use '+str(self._last_model_name))
                ...
                if not self._data_manager.crnn_first:
                    ...
                    self._model_sequences = [self._model_sequences[0]] + [CRNN_MODEL] \
                        + sorted(self._model_sequences[1:],
                                 key=lambda x: self._each_model_best_auc[x][-1], reverse=True)
                log(
                    'round '+str(self._round_num)+' start, model sequences '+str(self._model_sequences[self._model_idx:]))
            self._model_name = self._model_sequences[self._model_idx]
            self._model_num += 1
            self._not_rise_num = 0
            log(
                'change model from '+str(self._last_model_name)+' to '+str(self._model_name) + ', loop_num: '+str(self._cur_model_run_loop))
...
#             self._is_reset_model=self._use_new_train
            ###
            if self._use_new_train and not self.is_given_test:
                self._test_x = None
            self._cur_model_run_loop = 0
            ...
            if self._is_reset_model:
                log('new round '+str(self._round_num))
                # clear all models
                self._models.clear()
                del self._model
                self._model = None
    ...
    def _blending_ensemble(self):
        ...
        log("model_num: " + str(self._model_num) + " Select k best " + str(self._keep_num) + " predicts which have auc " + str(self._k_best_auc) +
            "\neach model " + str(self._each_model_keep_num) + " best which have auc " + str(each_model_k_aucs) +
            "and each previous model's best predict which have auc " +
            " ".join(['(%s:%s)' % (str(k), str(v)) for k, v in self._each_model_best_auc.items()]))
    ...
    def fit(self, train_loop_num=1, **kwargs):
        ...
        log('fit '+str(self._model_name)+' for '+str(self._cur_model_run_loop)+' times')
        ...
        log('train_x: '+str(train_x.shape)+'; train_y: '+ str(train_y.shape) +
            ' val_x: '+str(val_x.shape)+'; val_y: '+str(val_y.shape)+';')
    ...
    def predict(self, test_x, is_final_test_x=False):
        ...
        if auc > self._cur_model_max_auc:
            log(
                'cur_max_auc '+str(self._cur_model_max_auc)+'; cur_auc '+str(auc)+'; '+str(self._model_name)+' auc rise for '+str(self._cur_model_run_loop)+' times')
            ...
            log(
                'cur_max_auc '+str(self._cur_model_max_auc)+'; cur_auc '+str(auc)+'; '+str(self._model_name)+' auc not rise for '+str(self._not_rise_num)+' times')
        ...
        self._is_reset_model = False
        self.is_given_test=False
        self.last_predict=self._blending_ensemble()
