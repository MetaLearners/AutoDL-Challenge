...
from .data_process import extract_mfcc_parallel, get_max_length, pad_seq, extract_melspectrogram_parallel,extract_melspectrogram_parallel_librosa,extract_melspectrogram_parallel_cpu,extract_melspectrogram_parallel_sonopy
from .tools import log, timeit

import time
#@timeit
def audio_to_ndarray(x,wl):
    tmp=[]
    for i,sample in enumerate(x):
        sl=len(sample)
        if sl>=wl:
            tmp.append(sample[0:wl])
        else:
            repeat_times=wl//sl
            left=wl-repeat_times*sl
            tmp.append(np.concatenate([np.tile(sample,repeat_times),sample[:left]],axis=0))
    x=tmp
    x=np.asarray(x)
    return x
def lr_preprocess1( x):
        wl=MAX_AUDIO_DURATION * AUDIO_SAMPLE_RATE
        x=audio_to_ndarray(x,wl)
        ### for testing
#         x_mel = extract_melspectrogram_parallel_cpu(
#             x, n_mels=30, use_power_db=True)
#         x_mel = extract_melspectrogram_parallel_librosa(
#             x, n_mels=30, use_power_db=True)
#         x_mel = extract_melspectrogram_parallel_sonopy(
#             x, n_mels=30, use_power_db=True)
        ###
        x_mel = extract_melspectrogram_parallel(
            x, n_mels=30, use_power_db=True)
        
        mel_mean=np.mean(x_mel,axis=2)
        mel_std=np.std(x_mel,axis=2)
        x_feas=np.concatenate([mel_mean,mel_std],axis=-1)
        return x_feas
class DataManager:
...
#         self._all_x = np.array(self._all_x)
    ...
    def _init_each_class_index(self):
        ...
        log('Raw train data: train_num(without val) '+str(len(self._train_y))+'; ' +
            'class_num '+str(self._num_classes)+' ; max_class_num '+str(self._max_class_num)+'; min_class_num '+str(self._min_class_num)+'; ')
    ...
    def _train_test_split_index(self, ratio=0.8):
        ...
        try:
            ...
            print("--------------------------------------------------------------------")
            print(type(self._all_y))
            print(self._all_y.shape)
            print("--------------------------------------------------------------------")
    ...
    def _train_test_split(self, ratio=0.8):
        ...
        self._train2all=dict(zip(range(len(train_index)),train_index))
#         self._val2all=dict(zip(range(len(val_index)),val_index))
        self._val_index=valid_index
    ...
    def _get_preprocess_train(self, sample_index, pre_func):
        ...
        log('Total ' + str(len(sample_index)) + ', update '+str(len(need_pre)))
        # update
        cnt = 0
        for i in need_pre:
    ...
    def change_data(self,data,predata,continue_lr=True):
        self._all_x,self._all_y=data
        self._all_x=np.array(self._all_x)
        self._val_x =None
        if continue_lr:
            self.data_changed=True
            self._lr_val_x =None
            self._pre_data=predata
        print("-----------------------------------------change data")
    #@timeit
...
#                 lr_sample_num = 500
            self._init_even_class_index()
            lr_sample_num=len(self._all_y)
#             print("lr_sample_num :{}".format(lr_sample_num))
...
#                         np.int)
            train_samples_in_all=list(map(lambda x :self._train2all[x],train_samples))
#             print(type(train_samples_in_all))
#             print(train_samples_in_all)
#             print(type(self._pre_data))
#             print(self._pre_data.shape)
#             print(self._pre_data[0])
#             print(train_samples)
#             print(train_samples_in_all)
            self._lr_train_x=self._pre_data[train_samples_in_all,:]
            self._lr_train_y=self._all_y[train_samples_in_all,:]
            ###
            if self._lr_val_x is None:
#                 self._lr_val_x = self.lr_preprocess(self._val_x)
                self._lr_val_x=self._pre_data[self._val_index,:]
...
#         print("raw max : {}".format(self.raw_max_length))
        
        if is_mfcc:
                # extract mfcc
            x = extract_mfcc_parallel(x, n_mfcc=n_mfcc)
        else:
#             x = audio_to_ndarray(x,self.raw_max_length)
            x = extract_melspectrogram_parallel_librosa(
