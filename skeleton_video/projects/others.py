...
def get_tf_resize(height, width, original_times, times=1, min_value=0.0, max_value=1.0):
    def preprocessor(tensor):
        ...
        if width == in_width and height == in_height:
            ...
            tensor = tf.image.resize_images(tensor, (height, width), method=tf.image.ResizeMethod.BILINEAR )
            LOGGER.info('[get_tf_resize] resize from (%sx%s) to (%dx%d)', in_height, in_width, height, width)

        if False:
            # resize time axis using NN (to select frame \wo interpolate)
            LOGGER.info('tensor size %s', tensor.shape)
            ...
            tensor = tf.concat(values=[tensor, tensor, tensor], axis=0)
            tensor = tf.concat(values=[tensor, tensor], axis=0)
            try:
                tensor = tf.slice(tensor, [0,0,0,0], [original_times, -1, -1, -1])
            except:
                tensor = tf.concat(values=[tensor, tensor, tensor], axis=0)
                tensor = tf.slice(tensor, [0,0,0,0], [original_times, -1, -1, -1])

            LOGGER.info('tensor size %s slice', tensor.shape)
            offset = random.randint(0, 4)
            
            index = [5*i+offset for i in range(times)]
            LOGGER.info('offset %d, index %s', offset, index)
            tensor = tf.concat(values=[[tensor[i]] for i in index], axis=0)
            LOGGER.info('tensor size %s after index', tensor.shape)
