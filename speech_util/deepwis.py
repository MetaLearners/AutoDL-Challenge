...
from speech_util.nn.module import vggvox_resnet2d_icassp
import keras
import random
from collections import OrderedDict

import speech_util.data.data_utils_v2 as ut
import speech_util.data.generator_v2 as generator
from speech_util.utils.autospeech_eda import AutoSpeechEDA
from speech_util.utils.autospeech_sampler import AutoSpSamplerNew
...
from speech_util.tools import timeit
...
class Model2(object):
    def __init__(self, metadata, train_output_path="./", test_input_path="./"):
        ...
        self.train_num = self.metadata.size()
        self.class_num = self.metadata.get_output_size()
...
#         self.autospeech_sampler: AutoSpSamplerNew = None
    ...
    def change_multilabel(self,is_multi):
        if is_multi:
            self.config['loss']='sigmoid'
        else:
            self.config['loss']='softmax'
        pretrain_output = self.model.output
        ...
        if self.config['loss']=='softmax':
            ...
            y = keras.layers.Dense(
                    ...
                    kernel_regularizer=keras.regularizers.l2(weight_decay),
                    bias_regularizer=keras.regularizers.l2(weight_decay),
                    name="prediction_sigmoid",
                )(pretrain_output)
        model = keras.models.Model(self.model.input, y, name="vggvox_resnet2D_{}_{}_new".format("softmax", "gvlad"))
        if self.config['loss']=='softmax':
...
#         train_dataset= convert_speech(train_dataset)
