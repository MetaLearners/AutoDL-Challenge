...
class ResNet18(models.ResNet):
    ...
    def __init__(self, in_shape=[1,32,32,1], num_classes=10, **kwargs):
        ...
        self.simple_fc = torch.nn.Linear(self.simple_nn_num, num_classes, bias=False)
        self.cnt = 0
    ...
    def init(self, model_dir=None, gain=1.):
        ...
        # LOGGER.info('before load %s', model_dict['layer1.1.conv1.weight'])
        sd = {k: v for k, v in sd.items() if k in model_dict}
        self.load_state_dict(sd, strict=False)
        # LOGGER.info('load following params %s\n', sd.keys())
        # model_dict_after = self.state_dict()
        # LOGGER.info('after load %s', model_dict_after['layer1.1.conv1.weight'])
    ...
    def forward(self, inputs, targets=None, tau=8.0, reduction='avg'):  # pylint: disable=arguments-differ
        ...
        if self.use_linear_only and not self.is_video():
