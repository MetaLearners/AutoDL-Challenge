...
class LogicModel(Model):
    def __init__(self, metadata, session=None):
        ...
        self.timers = {
            'train': skeleton_video.utils.Timer(),
            'test': skeleton_video.utils.Timer()
        }
        self.info = {
            'dataset': {
                'path': self.metadata.get_dataset_name(),
                'shape': self.metadata.get_tensor_size(0),
                'original_shape':self.metadata.get_tensor_size(0),
        ...
        dataset_shape = self.info['dataset']['original_shape']
        if (dataset_shape[-2] > 0 and dataset_shape[-2] <= 32) and (dataset_shape[-3] > 0 and dataset_shape[-3] <= 32):
            self.image_mode = 'small_image'
        elif dataset_shape[-2] >= 200 or dataset_shape[-3] >= 200:
            self.image_mode = 'very_large_image'
        else:
            self.image_mode = 'large_image'
        ...
        self.hyper_params = {
            ...
            'conditions': {
                ...
                'test_after_at_least_seconds_step': 1,

                'threshold_valid_score_diff': 0.001,
                'threshold_valid_best_score': 0.999,
        ...
        self.start_original_size_epoch = 15
    ...
    def build_or_get_train_dataloader(self, dataset):
        ...
        train = skeleton_video.data.TFDataset(self.session, sample, num_samples)
        ...
        dataset_shape = self.info['dataset']['sample']['example']['shape']
        if (dataset_shape[-2] > 0 and dataset_shape[-2] <= 32) and (dataset_shape[-3] > 0 and dataset_shape[-3] <= 32):
            self.image_mode = 'small_image'
        elif dataset_shape[-2] >= 200 or dataset_shape[-3] >= 200:
            self.image_mode = 'very_large_image'
        ...
        LOGGER.info('[input shape avg %s',self.info['dataset']['sample']['example']['shape_avg'] )
        LOGGER.info('[input shape min %s',self.info['dataset']['sample']['example']['shape_min'] )
        LOGGER.info('[input shape max %s',self.info['dataset']['sample']['example']['shape_max'] )
        ...
        times_min = self.info['dataset']['sample']['example']['shape_min'][0]

        # preprocessor1 = get_tf_resize(64, 64, times=input_shape[0], min_value=values['min'], max_value=values['max'])
        preprocessor1 = get_tf_resize(input_shape[1], input_shape[2], times=input_shape[0], original_times=times, min_value=values['min'], max_value=values['max'])
        dataset = dataset.map(
    ...
    def build_or_get_dataloader(self, mode, dataset=None, num_items=0):
        ...
        LOGGER.info('[dataloader] %s build start', mode)
        values = self.info['dataset']['sample']['example']['value']
        channel_num = self.info['dataset']['sample']['example']['shape'][-1]
        times_num = self.info['dataset']['sample']['example']['shape'][0]
        ...
        if mode == 'train':
            ...
            dataset = skeleton_video.data.TFDataset(self.session, dataset, num_items)

            transform = tv.transforms.Compose([
                # lambda t: t.cpu().float() if isinstance(t, torch.Tensor) else torch.Tensor(t),
                # tv.transforms.ToPILImage(),
                # tv.transforms.RandomCrop(size = 64),
                # tv.transforms.RandomHorizontalFlip(p=0.3),
                # tv.transforms.RandomVerticalFlip(p=0.3),
                # tv.transforms.ToTensor(),
                # tv.transforms.Normalize(transform_mean, transform_std),
                skeleton_video.data.Cutout(int(input_shape[1] // 4), int(input_shape[2] // 4)),
                # lambda t: t.to(device=self.device) #.half()
            ])

            dataset = skeleton_video.data.TransformDataset(dataset, transform, index=0)

            self.dataloaders['train'] = skeleton_video.data.FixedSizeDataLoader(
                dataset,
            ...
            batch_size = self.hyper_params['dataset']['batch_size_test'] # =256
            input_shape = self.hyper_params['dataset']['input']
            # if self.use_original_size:
            #     input_shape = self.info['dataset']['sample']['example']['shape']
            preprocessor2 = get_tf_to_tensor(is_random_flip=False)
            if mode == 'valid':
                preprocessor = preprocessor2
            elif mode == 'test':
                preprocessor1 = get_tf_resize(input_shape[1], input_shape[2], original_times=times_num, times=input_shape[0], min_value=values['min'], max_value=values['max'])
                preprocessor = lambda *tensor: preprocessor2(preprocessor1(*tensor))
            
            # batch_size = 500
            # tf_dataset = dataset.apply(
            #     tf.data.experimental.map_batch(
            #         map_func=lambda *x: (preprocessor(x[0]), x[1]),
            #         batch_size=batch_size,
            #         drop_remainder=False,
            #         num_parallel_calls=tf.data.experimental.AUTOTUNE
            #     )
            # ).prefetch(buffer_size=batch_size)    # buffer_size=8
            dataset = dataset.map(
                lambda *x: (preprocessor(x[0]), x[1]),
                num_parallel_calls=tf.data.experimental.AUTOTUNE
            ...
            tmp_dataloader = torch.utils.data.DataLoader(
                dataset, batch_size=1, shuffle=False, drop_last=False, num_workers=0, pin_memory=False
            )
            tensors = []
            for example, label in tmp_dataloader:
                example = torch.Tensor(example)
                label = torch.Tensor(label)
                example.data = example.data.to(device=self.device)
                label.data = label.data.to(device=self.device)
                tensors.append([example, label])
            
            del tmp_dataloader
            tensors = [torch.cat(t, dim=0) for t in zip(*tensors)]
            dataset = skeleton_video.data.prefetch_dataset(tensors)
            transform = tv.transforms.Compose([
                ])
            dataset = skeleton_video.data.TransformDataset(dataset, transform, index=0)
            ...
            self.info['condition']['first']['test'] = False

        LOGGER.info('[dataloader] %s build end', mode)
        return self.dataloaders[mode]
    ...
    def update_condition(self, metrics=None):
        ...
        indices = sorted(indices[::-1][:self.hyper_params['checkpoints']['keep']])   # keep=50
    ...
    def break_train_loop_condition(self, remaining_time_budget=None, inner_epoch=1):
        ...
        lr = self.optimizer_all.param_groups[0]['lr']
        ...
        if self.info['loop']['epoch'] <= self.hyper_params['conditions']['early_epoch']:
            LOGGER.info('[BREAK] at %d epoch of early %d epoch', self.info['loop']['epoch'], self.hyper_params['conditions']['early_epoch'])
            return True

        if self.info['loop']['epoch'] < 20 and self.info['loop']['epoch'] % 3 == 0:
            LOGGER.info('[break] ')
            return True
        
        if self.info['loop']['epoch'] >= 20  and self.info['loop']['epoch'] < 40 and self.info['loop']['epoch'] % 4 == 0:
            LOGGER.info('[break] ')
            return True
        ...
        if self.info['loop']['epoch'] >= 25 and \
    ...
    def terminate_train_loop_condition(self, remaining_time_budget=None, inner_epoch=0):
        ...
        best_idx = np.argmax(np.array([c['valid']['score'] for c in self.checkpoints]))
        best_score = self.checkpoints[best_idx]['valid']['score']
        if best_score > self.hyper_params['conditions']['threshold_valid_best_score'] and self.info['loop']['epoch'] >= 20:
        ...
        if self.optimizer_all.param_groups[0]['lr'] < self.hyper_params['conditions']['min_lr']:
            LOGGER.info('[TERMINATE] lr=%f', self.optimizer_all.param_groups[0]['lr'])
            done = True if self.info['terminate'] else False
            self.info['terminate'] = True
            self.done_training = done
            return True

        # if self.info['loop']['epoch'] >= 40:
        #     LOGGER.info('[terminate] at epoch 40')
        #     self.info['terminate'] = True
        #     self.done_training = True
        #     return True
    ...
    def train(self, dataset, remaining_time_budget=None):
        ...
        # if epoch < self.start_original_size_epoch:
        #     train_dataloader = self.build_or_get_train_dataloader(dataset)
        # else:
        #     train_dataloader = self.rebuild_train_dataloader_with_original_size(dataset)
        ...
        while True:
            ...
            if train_score > self.hyper_params['conditions']['skip_valid_score_threshold'] or \
                self.info['loop']['test'] > 5: # self.hyper_params['conditions']['skip_valid_after_test']:
                ...
                if self.info['loop']['epoch'] > 5:
                    self.lr_schedular_all.step(valid_metrics['score'])
            else:
            ...
            lrs = [x['lr'] for x in self.optimizer_all.param_groups]
            LOGGER.info(
                '[train] [%02d] time(budge:%.2f, total:%.2f, step:%.2f) loss:(train:%.3f, valid:%.3f) score:(train:%.3f valid:%.3f) lr:%s',
        ...
        LOGGER.info(
            ...
            self.info['loop']['epoch'], remaining_time_budget, self.get_total_time(), self.timers['train'].step_time,
            metrics['train']['loss'], metrics['valid']['loss'], metrics['train']['score'], metrics['valid']['score'],
            self.optimizer_all.param_groups[0]['lr']
        )
        # LOGGER.info('[train] [%02d] Timer:%s', self.info['loop']['epoch'], self.timers['train'])

    def test(self, dataset, remaining_time_budget=None):
        LOGGER.info('[test][%02d] start', self.info['loop']['epoch'])
        self.timers['test']('start', exclude_total=True, reset_step=True)
        is_first = self.info['condition']['first']['test']
        self.info['loop']['test'] += 1

        if self.info['loop']['test'] < self.start_fc_epoch:
            LOGGER.info('TEST: %d epoch, train: %d epoch, use linear only',self.info['loop']['test'], self.info['loop']['epoch'])
            self.model.set_use_linear_only(True)
